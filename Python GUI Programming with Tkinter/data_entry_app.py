...
# global variables that the app will use to keep track of information
variables = dict()
records_saved = 0
...
# root window will only have one column, but by setting this it will allow the form to remain centered
# on the application if the window is expanded
root.columnconfigure(0, weight=1)
# add a heading for the application
# TkDefaultFont alias defined in Tkinter that points to the default window font on your platform
...
ttk.Entry(r_info, textvariable=variables["Date"]).grid(
...
ttk.Combobox(r_info, textvariable=variables["Time"], values=time_values).grid(
    row=1, column=1, sticky=(tk.W + tk.E)
)
# Technician
variables["Technician"] = tk.StringVar()
ttk.Label(r_info, text="Technician").grid(row=0, column=2)
ttk.Entry(r_info, textvariable=variables["Technician"]).grid(
